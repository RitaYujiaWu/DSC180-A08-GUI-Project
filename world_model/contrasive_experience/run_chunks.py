...
class ChunkRunner:
    ...
    def chunk_test_files(self, test_files: List[str]) -> List[List[str]]:
        ...
        if len(test_files) > 10000:
            self.chunk_size = 2000
        elif len(test_files) > 5000:
            self.chunk_size = 1000
        elif len(test_files) > 2000:
            self.chunk_size = 500
        elif len(test_files) > 1000:
            self.chunk_size = 250
        elif len(test_files) > 500:
            self.chunk_size = 100
    ...
    def run_chunk(self, chunk: List[str], chunk_idx: int, logger) -> Dict[str, Any]:
        ...
        elif self.args.evaluation_type in ["webvoyager", "expand_memory", "mind2web"]:
    ...
    def get_test_files_for_domain(self, domain: str, logger) -> List[str]:
        ...
        if self.args.evaluation_type == "mmina":
            ...
            test_file_list = glob.glob(os.path.join(f"data/benchmarks/mind2web/{domain}", "**", "*.json"), recursive=True)
            if not self.args.debug:
                test_file_list = get_unfinished(test_file_list, self.args.result_dir, 'data/benchmarks/mind2web')
            return test_file_list
