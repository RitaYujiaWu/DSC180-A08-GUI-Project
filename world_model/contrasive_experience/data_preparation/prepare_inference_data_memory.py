...
def get_inference_memory_embeddings(trajectory_path, dataset, domain):
    ...
    if dataset == 'mmina':
        ...
        config_path_list = [path for path in os.listdir(f'data/benchmarks/mind2web/{domain}') if path.endswith('.json')][:100]
        for config_path in config_path_list:
            with open(f'data/benchmarks/mind2web/{domain}/{config_path}', 'r') as f:
                config = json.load(f)
                question = config['intent']
                process_single_file(question, dataset, domain, 'qwen2.5-vl-32b', memory)
    elif dataset == 'webvoyager':
        config_path_list = [path for path in os.listdir(f'data/benchmarks/webvoyager/{domain}') if path.endswith('.json')]
        for config_path in config_path_list:
            with open(f'data/benchmarks/webvoyager/{domain}/{config_path}', 'r') as f:
                config = json.load(f)
                question = config['intent']
                process_single_file(question, dataset, domain, 'qwen2.5-vl-32b', memory)
