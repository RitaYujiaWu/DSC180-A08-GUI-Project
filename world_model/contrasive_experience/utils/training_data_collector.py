...
class TrainingDataCollector:
    ...
    "}</res_dict>"
            ...
            traceback.print_exc()
            # Use the actual score when evaluation server fails
            correctness = (score == 1) if score is not None else False
            return {
                "analysis": f"Evaluation error: {str(e)}",
                "evaluation": {"Correctness": correctness, "Error_Type": f"Evaluation error: {str(e)}"},
                "raw_response": ""
            }
    ...
    def end_conversation(self, conversation_summary: Optional[Dict[str, Any]] = None, score: Optional[int] = None) -> str:
        ...
        # NOTE: For PASS trajectories (score == 1), we don't need the external evaluator
        # (which may be served via a local OpenAI-compatible endpoint). We treat PASS as
        # correct and skip the evaluator to avoid noisy connection errors and latency.
        if score == 1:
            evaluation_result = {
                "analysis": "Skipped trajectory evaluation (score == 1).",
                "evaluation": {"Correctness": True},
                "raw_response": "",
            }
        else:
