...
def get_unfinished(config_files: List[str], result_dir: str, config_dir: str=None) -> List[str]:
    ...
    for config_file in config_files:
        with open(config_file, 'r') as f:
            config = json.load(f)
            # Handle both formats: MMInA uses 'task_id', WebVoyager uses 'id'
            task_id = str(config.get('task_id') or config.get('id', ''))
...
def create_test_file_list_visualwebarena(domain: str, start_idx: int = 0, end_idx: int = None) -> list:
    ...
    all_test_files = glob.glob(f"benchmarks/visualwebarena_evaluation/vwa/{domain}/*.json")
    task_num = len(all_test_files)
    if end_idx is None:
        end_idx = task_num - 1
    
    test_file_list = [f"benchmarks/visualwebarena_evaluation/vwa/{domain}/{i+1}.json" for i in range(start_idx, min(end_idx, task_num))]
    return test_file_list
...
def create_test_file_list_webarena(domain: str, start_idx: int = 0, end_idx: int = None) -> list:
    ...
    Create a list of test files for a domain
    """
    all_test_files = glob.glob(f"benchmarks/visualwebarena_evaluation/wa/{domain}/*.json")
    task_num = len(all_test_files)
    if end_idx is None:
        end_idx = task_num - 1
...
def create_test_file_list_webvoyager(domain: str, start_idx: int = 0, end_idx: int = None) -> list:
    ...
    Returns:
        List of test file paths
    """
    data_root = 'data/benchmarks/webvoyager'
    all_items = os.listdir(data_root) 
    all_domains = [item for item in all_items if os.path.isdir(os.path.join(data_root, item))]
    # if domain not in all_domains and domain != 'all':
    #     raise ValueError(f"Invalid domain: {domain}, available domains: {all_domains}")
    
    if domain == 'all':
        all_test_files = []
        all_domains = ['Amazon', 'Allrecipes', 'Google_Map', 'Coursera']
        for domain in all_domains:
            if domain not in ['test', 'search']:
                all_test_files.extend(glob.glob(f"{data_root}/{domain}/*.json"))
        all_test_files = [item for item in all_test_files if '--' in item]
    elif domain == 'check':
        with open('/home/wenyi1/rl_results/train/memory_agent_rl_v1/difficult_set.jsonl', 'r') as f:
            all_task_ids = [json.loads(line)['task_id'] for line in f]
        all_test_files = [f"/home/wenyi1/GUI-Agent-Learn-From-Error/CoMEM-Agent-Inference/results/webvoyager/qwen2.5-vl/test/test/related_queries_level1_V2/{task_id}.json" for task_id in all_task_ids]
    else:
        all_test_files = glob.glob(f"{data_root}/{domain}/*.json")
...
def create_test_file_list_expand_memory(domain: str, start_idx: int = 0, end_idx: int = None) -> list:
    ...
    """
    test_file_list = []
    with open(f"memory_evolution/generated_tasks/tasks_{domain}.json", 'r') as f:
        tasks = json.load(f)
    for idx, task in enumerate(tasks):
        ...
        config_folder = f"memory_evolution/generated_tasks_seperate/{domain}"
