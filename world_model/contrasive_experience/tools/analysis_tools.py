...
class PageParserTool:
    ...
    async def call(self, page, **kwargs) -> str:
        """Parse the current page content"""
        try:
            # Get the current page URL
            url = page.url
            # Use AsyncWebCrawler - now we can await directly
            parsed_web = await self._get_page_content_async(url)
            return parsed_web

        except Exception as e:
            return f"Error in page parser tool: {str(e)}"
    
    async def _get_page_content_async(self, url: str) -> str:
        """Async helper method to get page content"""
        try:
            ...
            if hasattr(self, 'llm') and self.llm:
                for img_info in image_analysis:
                    try:
                        ...
                        response, _, _ = self.llm.chat(messages=messages, stream=False)
                        ...
                        img_info['ai_description'] = ai_description
...
class ContentAnalyzerTool:
    ...
    async def _parse_page_content(self, page, query: str = "") -> str:
        """Internal method to parse page content using AsyncWebCrawler and LLM summarization"""
    ...
    async def call(self, args: str, **kwargs) -> str:
        ...
        page_content = await self._parse_page_content(page, query)
