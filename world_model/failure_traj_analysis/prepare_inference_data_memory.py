...
def get_inference_memory_embeddings(trajectory_path, dataset, domain):
    ...
    if dataset == 'mmina':
        ...
        config_path_list = [path for path in os.listdir(f'benchmarks/webvoyager_evaluation/data/{domain}') if path.endswith('.json')]
        for config_path in config_path_list:
            with open(f'benchmarks/webvoyager_evaluation/data/{domain}/{config_path}', 'r') as f:
                config = json.load(f)
                question = config['intent']
                process_single_file(question, dataset, domain, 'qwen2.5-vl-32b', memory)
